...
async def test_state_update(hass):
    ...
    assert hass.states.is_state("binary_sensor.vaillant_circulation", "off")
    ...
    system.circulation = Circulation(
        "circulation", "Circulation", time_program(SettingModes.ON), OperatingModes.AUTO
    )
    ...
    assert_entities_count(hass, 10)
    assert hass.states.is_state("binary_sensor.vaillant_circulation", "on")
    ...
    state = hass.states.get("binary_sensor.vaillant_holiday")
    assert state.attributes["start_date"] == system.holiday_mode.start_date.isoformat()
    assert state.attributes["end_date"] == system.holiday_mode.end_date.isoformat()
    assert state.attributes["temperature"] == system.holiday_mode.target_temperature
    assert hass.states.is_state("binary_sensor.vaillant_quick_mode", "off")
