...
from tests.components.vaillant import setup_vaillant, SystemManagerMock, \
    call_service, get_system, active_holiday_mode
...
async def test_valid_config(hass):
    ...
    assert len(hass.services.async_services()[DOMAIN]) == 6
...
async def test_remove_quick_mode_wrong_data(hass):
    ...
    with pytest.raises(voluptuous.error.MultipleInvalid):
        await call_service(hass, 'vaillant', 'remove_quick_mode',
                           {'test': 'boom'})
...
async def test_remove_holiday_mode_wrong_data(hass):
    ...
    assert await setup_vaillant(hass)
    with pytest.raises(voluptuous.error.MultipleInvalid):
        await call_service(hass, 'vaillant', 'remove_holiday_mode',
                           {'test': 'boom'})
...
async def test_set_quick_mode(hass):
    """Set quick mode."""
    assert await setup_vaillant(hass)
    await call_service(hass, 'vaillant', 'set_quick_mode',
                       {'quick_mode': 'QM_PARTY'})
...
async def test_set_holiday_mode(hass):
    """Test holiday mode."""
    assert await setup_vaillant(hass)
    await call_service(hass, 'vaillant', 'set_holiday_mode', {
...
async def test_remove_quick_veto_wrong_data(hass):
    ...
    assert await setup_vaillant(hass)
    await call_service(hass, 'vaillant', 'remove_quick_veto',
                       {'entity_id': 'climate.vaillant_test123'})
...
async def test_remove_quick_veto_room(hass):
    ...
    system = get_system()
    system.rooms[0].quick_veto = QuickVeto(10, 10)
    assert await setup_vaillant(hass, system=system)
    await call_service(hass, 'vaillant', 'remove_quick_veto',
                       {'entity_id': 'climate.vaillant_room_1'})
    SystemManagerMock.instance \
        .remove_room_quick_veto.assert_called_once_with('1')
...
async def test_remove_quick_veto_zone(hass):
    """Remove quick veto with already existing quick veto."""
    system = get_system()
    system.zones[0].quick_veto = QuickVeto(10, 10)
    assert await setup_vaillant(hass, system=system)
    await call_service(hass, 'vaillant', 'remove_quick_veto',
                       {'entity_id': 'climate.vaillant_zone_1'})
    SystemManagerMock.instance. \
        remove_zone_quick_veto.assert_called_once_with('zone_1')


async def test_set_quick_veto_room(hass):
    """Set quick veto without quick veto."""
    assert await setup_vaillant(hass)
    await call_service(hass, DOMAIN, 'set_quick_veto',
                       {
                           'entity_id': 'climate.vaillant_room_1',
                           'duration': 300,
                           'temperature': 25
    ...
    SystemManagerMock.instance. \
        set_room_quick_veto.assert_called_once_with('1', QuickVeto(300, 25.0))
...
async def test_set_quick_veto_zone_with_quick_veto(hass):
    ...
    SystemManagerMock.instance. \
        set_zone_quick_veto.assert_called_once_with('zone_1',
                                                    QuickVeto(300, 25.0))
    SystemManagerMock.instance. \
        remove_zone_quick_veto.assert_called_once_with('zone_1')
