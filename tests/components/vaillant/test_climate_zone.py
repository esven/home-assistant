...
async def _test_mode_hvac(hass, mode, hvac_mode, target_temp):
    ...
    zone = SystemManagerMock.system.zones[0]
    _assert_zone_state(hass, mode, hvac_mode, zone.current_temperature, target_temp)
...
async def test_day_mode_hvac_heat(hass):
    ...
    await _test_mode_hvac(
        hass, OperatingModes.DAY, HVAC_MODE_HEAT, zone.target_temperature
    )
...
async def test_night_mode_hvac_cool(hass):
    ...
    zone = get_system().zones[0]
    await _test_mode_hvac(
        hass, OperatingModes.NIGHT, HVAC_MODE_COOL, zone.target_min_temperature
    )
...
async def test_auto_mode_hvac_auto(hass):
    ...
    zone = get_system().zones[0]
    await _test_mode_hvac(
        hass, OperatingModes.AUTO, HVAC_MODE_AUTO, zone.active_mode.target_temperature
    )
...
async def test_quickmode_party_mode_hvac_heat(hass):
    ...
    zone = get_system().zones[0]
    await _test_mode_hvac(
        hass, QuickModes.PARTY, HVAC_MODE_HEAT, zone.target_temperature
    )
...
async def test_quickmode_one_day_home_hvac_auto(hass):
    ...
    zone = get_system().zones[0]
    await _test_mode_hvac(
        hass, QuickModes.ONE_DAY_AT_HOME, HVAC_MODE_AUTO, zone.target_temperature
    )
...
async def test_holiday_hvac_off(hass):
    ...
    assert await setup_vaillant(hass, system=system)
    _assert_zone_state(
        hass, QuickModes.HOLIDAY, HVAC_MODE_OFF, system.zones[0].current_temperature, 15
    )
...
async def test_set_target_temp_cool(hass):
    ...
    _assert_zone_state(
        hass, OperatingModes.QUICK_VETO, HVAC_MODE_COOL, zone.current_temperature, 14
    )
...
async def test_set_target_temp_heat(hass):
    ...
    _assert_zone_state(
        hass, OperatingModes.QUICK_VETO, HVAC_MODE_HEAT, zone.current_temperature, 30
    )
    SystemManagerMock.instance.set_zone_quick_veto.assert_called_once()
